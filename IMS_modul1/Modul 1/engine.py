            ...
            if (row_toko[0] == row_bank[0]):
                if (row_toko[5] < row_bank[5]):
                    if (row_toko[1] != row_bank[1]):
                        cursor_toko.execute("UPDATE tb_transaksi SET nama_member = '%s' WHERE id_transaksi = %s" % (
                        row_bank[1], row_toko[0]))
                        print("")
                        db_toko.commit()
                    if (row_toko[2] != row_bank[2]):
                        cursor_toko.execute("UPDATE tb_transaksi SET no_transaksi = '%s' WHERE id_transaksi = %s" % (
                        row_bank[2], row_toko[0]))
                        db_toko.commit()
                    if (row_toko[3] != row_bank[3]):
                        cursor_toko.execute("UPDATE tb_transaksi SET status_transaksi  = '%s' WHERE id_transaksi = %s" % (
                        row_bank[3], row_toko[0]))
                        db_toko.commit()
                    cursor_bank.execute("UPDATE tb_transaksi_toko SET updated_at = '%s' WHERE id_transaksi_toko = %s" % (
                    row_bank[5], row_toko[0]))
                    db_bank.commit()
