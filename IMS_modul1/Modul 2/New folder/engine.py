...
while True:
    ...
    if (increment_bank[0]) < (increment_toko[0]):
        ...
        for json_toko in data:
            # print(json_toko['status_json'], json_toko['status_sinkron'])
            if json_toko['status_json'] == 'insert' and json_toko['status_sinkron'] == True:
                print("Terjadi penambahan data bank")
                print(json_toko['id_transaksi'], json_toko['nama_member'], json_toko['no_transaksi'], json_toko['status_transaksi'], json_toko['tanggal_transaksi'], json_toko['updated_at'])
                id_transaksi, nama_member, no_transaksi, status_transaksi, tanggal_transaksi, updated_at = json_toko['id_transaksi'], json_toko['nama_member'], json_toko['no_transaksi'], json_toko['status_transaksi'], json_toko['tanggal_transaksi'], json_toko['updated_at']
                cursor_bank.execute("INSERT INTO tb_transaksi_toko VALUES(%s, '%s', '%s', '%s', '%s', '%s')" % (id_transaksi, nama_member, no_transaksi, status_transaksi, tanggal_transaksi, updated_at))
                db_bank.commit()

                openfilee = open("text.json", "r+")
    ...
    if (data_cur_toko1[0] < data_cur_bank2[0]):
        ...
        for json_toko in data:
            ...
            if json_toko['status_json'] == 'delete' and json_toko['status_sinkron'] == True:
                ...
                # print(json_toko['id_transaksi'], json_toko['nama_member'], json_toko['no_transaksi'], json_toko['status_transaksi'], json_toko['tanggal_transaksi'], json_toko['updated_at'])
                cursor_bank.execute("DELETE FROM tb_transaksi_toko WHERE id_transaksi_toko = %s" % json_toko['id_transaksi'])
                db_bank.commit()
